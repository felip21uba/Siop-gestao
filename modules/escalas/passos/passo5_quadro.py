...
import datetime, calendar, pandas as pd, copy, re
...
def renderizar_passo5():
    ...
    eh_admin = any(p in str(st.session_state.get("usuario_dados", {}).get("cargo_funcao", "")).upper() or p in str(st.session_state.get("usuario_dados", {}).get("perfil", "")).upper() for p in ["PROGRAMADOR", "TESTADOR", "ADMIN", "DESENVOLVEDOR"])
    ...
    with st.expander("📌 PASSO 5: Quadro Mensal de Escalas e Carga Horária", expanded=True):
        ...
        with st.expander("⚡ Painel de Ajuste Rápido no Quadro (Lançamento em Lote)", expanded=False):
            if mils_ord and not quadro_travado:
                ...
                mils_sel_lote = c_f1.multiselect("Militar(es):", list(dict_mils.keys()), key="p5_lote")
                dias_alvo = c_f2.multiselect("Dia(s):", list(range(1, num_dias + 1)), key="p5_dias")
