...
def verificar_trava_sobreposicao():
    # Se o operador clicou no OK para fechar/limpar, ignora e limpa até o próximo clique em Aplicar
    ...
    mils_map = {
        str(m.get("id")): f"{padronizar_graduacao(m.get('posto_grad'))} {m.get('nome_guerra')}"
        for m in mils
    }
    ...
    for m_id, nome_mil in mils_map.items():
        ...
        for d in range(1, num_dias + 1):
            ...
            eqs_mil = [
                ...
                if isinstance(p, (tuple, list)) and str(p[0]) == m_id
            ]
            ...
            for eq in eqs_mil:
                ...
                if val_clean in SIGLAS_DIAS_NEUTROS or val_clean in ["", "NONE", "NAN"]:
                    continue
        ...
        for i in range(len(todos_intervalos_mil)):
            for j in range(i + 1, len(todos_intervalos_mil)):
                ...
                if t1["inicio"] < t2["fim"] and t1["fim"] > t2["inicio"]:
                    bloqueios.append({
                        "militar": nome_mil,
                        "mensagem": f"Choque no Dia {t1['dia']:02d}/{m_mes:02d}: [{t1['equipe']}] {t1['texto_raw']} e [{t2['equipe']}] {t2['texto_raw']} se sobrepõem!"
                    })
            ...
            if proximo["inicio"] >= atual["fim"]:
                ...
                if diferenca_horas < 6.0:
                    avisos_descanso.append({
                        "militar": nome_mil,
                        "mensagem": f"Descanso de apenas {diferenca_horas:.1f}h entre o dia {atual['dia']:02d} e o dia {proximo['dia']:02d} (mínimo exigido: 6h)."
                    })
...
def renderizar_passo5():
    ...
    with st.expander("📌 PASSO 5: Quadro Mensal de Escalas e Carga Horária", expanded=True):
        ...
        with col_btn:
            # AO CLICAR EM APLICAR: RESETAR FLAG DE LIMPEZA E LER O QUADRO NOVAMENTE
        ...
        with col_link:
            url_espelho = f"?espelho=true&mes={m_mes}&ano={m_ano}"
            st.link_button("🖥️ Abrir 2ª Tela", url_espelho, use_container_width=True, help="Abre o Quadro 5 em uma janela separada.")
        ...
        if (bloqueios or avisos_descanso) and not st.session_state.get("limpar_avisos_manual", False):
            ...
            with c_btn_fechar:
                if st.button("✖ OK / Entendido", type="secondary", use_container_width=True, key="btn_limpar_avisos_p5"):
                    # LIMPA DA MEMÓRIA EXPLICITAMENTE
                    st.session_state["lista_bloqueios_auditoria"] = []
                    st.session_state["lista_avisos_descanso"] = []
