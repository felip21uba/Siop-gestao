...
def extrair_intervalos_horarios(texto_celula, data_ref):
    ...
    padrao = re.findall(
        r'(\d{1,2}:\d{2})\s*(?:ÀS|AS|-|A)\s*(\d{1,2}:\d{2})',
        texto
    )
...
def calcular_horas_efetivas_turno(
    texto_celula,
    data_ref,
    eh_supervisao=False
):
    intervalos = extrair_intervalos_horarios(
        texto_celula,
        data_ref
    )
    ...
    if (
        eh_supervisao
        or "SUPERVISÃO" in str(texto_celula).upper()
    ):
        horas_presenciais_reais = (
            minutos_presenciais_reais / 60.0
        )

        horas_sobreaviso_restantes = max(
            0.0,
            24.0 - horas_presenciais_reais
        )

        credito_sobreaviso = (
            horas_sobreaviso_restantes * 0.25
        )
...
def executar_auto_save_banco():
    ...
    try:
        ...
        salvar_escala_mensal_supabase(
            ...
            equipe_nome=st.session_state.get(
                "equipe_ativa",
                "GERAL"
            ),

            modalidade=st.session_state.get(
                "modalidade_turno_ativa",
                "Turno Único / Avulso"
            ),
...
def carregar_escala_salva_banco():
    ...
    try:
        ...
        print(
            f"Aviso ao carregar do banco: {ex}"
        )
...
def verificar_trava_sobreposicao():
    ...
    chaves = st.session_state.get(
        "militares_no_quadro_chaves",
        []
    )

    mils = st.session_state.get(
        "lista_militares",
        []
    )
    ...
    mils_map = {
        ...
        str(m.get("id")):
            f"{padronizar_graduacao(m.get('posto_grad'))} "
            f"{m.get('nome_guerra')}"
    ...
    for m_id, nome_mil in mils_map.items():
        ...
        for d in range(1, num_dias + 1):
            ...
            for eq in eqs_mil:
                ...
                val = grade.get(
                    f"{m_id}_{eq}_{m_ano}_{m_mes:02d}_{d:02d}",
                    "F"
                )
                ...
                if (
                    val_clean in SIGLAS_DIAS_NEUTROS
                    or val_clean in [
                        "",
                        "NONE",
                        "NAN"
        ...
        todos_intervalos_ord = sorted(
            todos_intervalos_mil,
            key=lambda x: x["inicio"]
        )

        for i in range(
            len(todos_intervalos_ord) - 1
        ):
            ...
            if proximo["inicio"] >= atual["fim"]:
                ...
                if diferenca_horas < 6.0:
                    ...
                    avisos_descanso.append({
                        ...
                        "mensagem":
                            f"Descanso de apenas "
                            f"{diferenca_horas:.1f}h "
                            f"entre o dia "
                            f"{atual['dia']:02d} "
                            f"e o dia "
                            f"{proximo['dia']:02d} "
    ...
    st.session_state[
        "lista_bloqueios_auditoria"
    ] = bloqueios

    st.session_state[
        "lista_avisos_descanso"
    ] = avisos_descanso
...
def recalcular_escala_matriz():
    ...
    h_avulso = st.session_state.get(
        "horario_avulso_p2",
        "07:00 às 19:00"
    )

    h_adm_norm = st.session_state.get(
        "adm_h_norm",
        "08:00 às 12:00\n13:30 às 17:00"
    )

    h_adm_qua = st.session_state.get(
        "adm_h_qua",
        "08:30 às 13:00"
    )
    ...
    sem_iso_d1 = datetime.date(
        m_ano,
        m_mes,
        1
    ).isocalendar()[1]

    for pair in st.session_state.get(
        "militares_no_quadro_chaves",
        []
    ):
        ...
        for d in range(
            ...
            if any(
                ...
                for sig in SIGLAS_DIAS_NEUTROS
...
def abrir_segunda_janela(m_mes, m_ano):
    ...
    components.html(
        ...
        const esquerda =
            Math.max(
                0,
                screen.availWidth - largura
            ) / 2;

        const topo =
            Math.max(
                0,
                screen.availHeight - altura
            ) / 2;
...
def renderizar_passo5():
    ...
    if val_espelho in [
        ...
        from modules.escalas.passos.passo5_segunda_tela import (
    ...
    if st.session_state.get(
        "atualizar_quadro_passo5",
        False
    ):

        sel_ids = set(
            str(mid)
            for mid in st.session_state.get(
                "militares_selecionados_ids",
                []
            )
        )
    ...
    bloqueios = st.session_state.get(
        "lista_bloqueios_auditoria",
        []
    )

    avisos_descanso = st.session_state.get(
        "lista_avisos_descanso",
        []
    )
    ...
    militares = (
        ...
        or carregar_militares_supabase()
        or []
    )
    ...
    quadro_travado = st.session_state.get(
        "toggle_trava_quadro",
        False
    )

    with st.expander(
        "📌 PASSO 5: Quadro Mensal de Escalas e Carga Horária",
        expanded=True
    ):
        ...
        col_esq, col_btn, col_link = st.columns(
            [2.0, 1.2, 0.8],
        ...
        with col_esq:
            ...
            st.markdown(
                f"""
                👮‍♂️ **Linhas Ativas:** `{cnt_linhas}`
                &nbsp;|&nbsp;
                💡 *Legenda `X` = serviço em outra equipe.*
                """
            )
        ...
        with col_link:
            ...
            if st.button(
                "🖥️ Abrir 2ª Tela",
                use_container_width=True,
                help="Abre o Quadro 5 em uma janela separada."
            ):
        ...
        if bloqueios or avisos_descanso:

            st.markdown("---")

            c_head_av, c_btn_fechar = st.columns(
                [4, 1]
            )
            ...
            with c_btn_fechar:
                ...
                if st.button(
                    ...
                    st.toast(
                        "🧹 Quadro de avisos ocultado temporariamente!",
                        icon="✅"
                    )
        ...
        chaves_existentes = st.session_state.get(
            "militares_no_quadro_chaves",
            []
        )
        ...
        with st.expander(
            "⚡ Painel de Ajuste Rápido no Quadro (Lançamento em Lote)",
            expanded=False
        ):
            ...
            if mils_ord and not quadro_travado:
                ...
                mils_sel_lote = c_f1.multiselect(
                ...
                if (
                    "Horário Normal" in tipo_ev
                    or "DNT" in tipo_ev
                ):

                    c_h1, c_h2, c_btn = st.columns(
                        [1.5, 1.5, 3]
                    )
                    ...
                    with c_btn:
                        ...
                        btn_aplicar_lote = st.button(

                            "⚡ Aplicar Alteração Direta",

                            type="primary",

                            use_container_width=True,

                            key="btn_aplicar_lote_norm"
                        )
        ...
        colunas_dias = [
            ...
            (
                ...
                (
                    f"{'🔴 ' if calendar.weekday(m_ano, m_mes, d) in [5,6] else ''}"
                    f"{d:02d} "
                    f"{DIAS_SEMANA_SIGLAS[calendar.weekday(m_ano, m_mes, d)]}"
                )
            )
        ...
        for idx_r, item in enumerate(
            mils_ord
        ):

            m_id = item["id"]
            eq = item["equipe"]
            pg = padronizar_graduacao(
                item["posto_grad"]
            )
            ng = item["nome_guerra"]
            np = item["num_policia"]
            ...
            for d, col_name in colunas_dias:
                ...
                v = padronizar_entrada_quadro(

                    grade.get(

                        f"{m_id}_{eq}_"
                        f"{m_ano}_{m_mes:02d}_"
                        f"{d:02d}",
                ...
                if (
                    ...
                    tot_h += (
                        calcular_horas_efetivas_turno(
                            v_str,
                            dt_ref_dia,
                            eh_supervisao=eh_sup
                        )
                    )
        ...
        if not df_escala.empty:
            ...
            # IMPORTANTE:
            # salva automaticamente qualquer edição
            # feita diretamente no data_editor.
            if alterou_quadro:
