    ...
    with st.expander("📌 PASSO 6: Visualização da Escala e Exportação Oficial", expanded=True):
        ...
        with st.expander("➕ ⚙️ Configurações de Emissão, Assinaturas e Importação Externa", expanded=False):
            ...
            with c_cfg1:
                ...
                st.caption("Já possui uma escala pronta em Excel? Importe-a diretamente para o sistema.")
                if st.button("📥 Importar Escala Pronta (.xlsx)", type="secondary", use_container_width=True):
                    try:
                        from modules.escalas.passos.passo5_quadro import abrir_modal_importar_escala_excel
                        abrir_modal_importar_escala_excel()
                    except ImportError:
                        st.error("A funcionalidade de importação de Excel está no Passo 5.")
