...
# ==============================================================================
# 1. FUNÇÕES DE HIGIENIZAÇÃO E DEDUPLICAÇÃO
# ==============================================================================
...
def chave_ferias_unica(reg):
    """Gera chave estável por MILITAR + PERÍODO DE FÉRIAS."""
...
def processar_txt_sirh(file_bytes):
    """Lê o TXT do SIRH extraindo com precisão a LOTAÇÃO/FRAÇÃO COMPLETA."""
    try:
        try:
        ...
        for _, row in df.iterrows():
            ...
            for candidato in candidatos_lotacao:
                cand_clean = normalizar_texto(candidato)
                if cand_clean and cand_clean not in ["NAN", "NONE", "N/I"]:
                    lotacao_detalhada = cand_clean
                    break
...
def extrair_texto_ferias(texto_bruto):
    """Extrai férias a partir de textos copiados ou relatórios PDF."""
    lista_ferias = []
...
def salvar_ferias_supabase_lote(lista_dados):
    """Salva no Supabase garantindo que nenhum período duplicado seja inserido."""
...
def carregar_ferias_supabase(ano=None, mes=None, busca="", lotacao_sel="Todas", filtro_dias="Todos"):
    """Consulta férias com desduplicação e cruzamento com o cadastro unificado de militares."""
    if not supabase:
        return []
    try:
        ...
        for row in dados_brutos:
            ...
            if lotacao_sel and lotacao_sel != "Todas":
                if normalizar_texto(lotacao_sel) not in row.get("lotacao", ""):
                    continue
...
# ==============================================================================
# 5. ESTRUTURA UNIFICADA DA INTERFACE DO PASSO 8 (SINGLE EXPANDER)
# ==============================================================================

def renderizar_modulo_ferias_anual():
    # 🟢 EXPANDER ÚNICO DO PASSO 8
    with st.expander("📌 PASSO 8: Mapeamento Anual de Férias & Indisponibilidade", expanded=True):
        ...
        # 🟢 NAVEGAÇÃO INTERNA POR ABAS (TABS)
        tab_consulta, tab_importacao = st.tabs([
            "🔍 Consulta e Gestão de Férias", 
            "📥 Importar Arquivo (SIRH TXT / PDF)"
        ])

        # ----------------------------------------------------------------------
        # ABA 1: CONSULTA, FILTROS E TABELA EDITÁVEL
        # ----------------------------------------------------------------------
        with tab_consulta:
            ...
            if supabase:
                try:
                    res_u = supabase.table("plano_ferias_anual").select("unidade").execute()
                    if res_u and res_u.data:
                        for it in res_u.data:
                            u_norm = normalizar_texto(it.get("unidade", ""))
                            if u_norm and u_norm not in ["NONE", "NAN", "N/I", ""]:
                                unids_set.add(u_norm)
                except Exception:
                    pass

            mils_cad = carregar_militares_supabase() or []
            for m in mils_cad:
                if isinstance(m, dict) and m.get("unidade"):
                    u_m = normalizar_texto(m["unidade"])
                    if u_m and u_m not in ["NONE", "NAN", "N/I", ""]:
                        unids_set.add(u_m)

            if unids_set:
                opcoes_lotacao += sorted(list(unids_set))
            ...
            with c1:
                v_ano = st.number_input("Ano:", min_value=2024, max_value=2035, value=st.session_state.get("ano_escala", datetime.date.today().year), key="p8_f_ano_v19")
            with c2:
                m_nomes = ["Todos", "Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"]
                v_mes_nome = st.selectbox("Mês de Referência:", m_nomes, key="p8_f_mes_v19")
                v_mes_num = m_nomes.index(v_mes_nome)
            with c3:
                v_dias = st.selectbox("Duração (Dias):", ["Todos", "10 dias", "15 dias", "20 dias", "25 dias", "30 dias"], key="p8_f_dias_v19")
            with c4:
                v_lotacao = st.selectbox("Lotação / Fração Detalhada:", opcoes_lotacao, key="p8_f_lot_v19")

            v_busca = st.text_input("🔎 Buscar Militar (Nome de Guerra, Servidor ou Nº Polícia):", placeholder="Digite o nome ou a matrícula...", key="p8_f_busca_v19").strip().upper()
            ...
            with col_limp2:
                if st.button("🗑️ Limpar Arquivos de Férias", type="secondary", use_container_width=True, key="p8_btn_limpar_v19"):
        ...
        # ----------------------------------------------------------------------
        # ABA 2: UPLOAD DE ARQUIVOS E TEXTO
        # ----------------------------------------------------------------------
        with tab_importacao:
            ...
            with u_col1:
                ...
                if arq_up is not None:
                    ...
                    if ext.endswith((".txt", ".csv")):
                        if st.button("🚀 Processar TXT/CSV (SIRH)", type="primary", use_container_width=True, key="btn_proc_sirh_v19"):
                        ...
                        if st.button("🚀 Extrair do PDF", type="primary", use_container_width=True, key="btn_proc_pdf_v19"):
                            st.session_state.pop("p8_temp_ferias", None)
                            try:
                                import pypdf
            ...
            with u_col2:
                ...
                txt_copiado = st.text_area("Cole a nota aqui:", placeholder="Ex: 1337468 SD SILVA - 10 DIAS A PARTIR DE 10/02/2026", height=110, key="p8_txt_area_v19")
                if st.button("⚡ Processar Texto Copiado", use_container_width=True, key="btn_proc_txt_v19"):
                    st.session_state.pop("p8_temp_ferias", None)
            ...
            if st.session_state.get("p8_temp_ferias"):
                ...
                if st.button("💾 Gravar Registros no Banco de Dados", type="primary", use_container_width=True, key="btn_salvar_db_v19"):
